    ...
    <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id="bar" x1="0" y1="0" x2="1" y2="0">
          ...
          <stop offset="${yesPercent}%" stop-color="#22c55e"/>
          <stop offset="${yesPercent}%" stop-color="#ef4444"/>
      ...
      <text x="1080" y="74" font-family="Arial, sans-serif" font-size="14" font-weight="bold" fill="${statusColor}" text-anchor="middle">${statusLabel}</text>
      <!-- Category -->
      <rect x="60" y="110" width="${market.category.length * 12 + 24}" height="30" rx="15" fill="#333"/>
      <text x="72" y="130" font-family="Arial, sans-serif" font-size="13" fill="#aaa">${market.category}</text>
      <!-- Title -->
      <text x="60" y="200" font-family="Arial, sans-serif" font-size="40" font-weight="bold" fill="#fff">
        ${market.title.length > 40 ? market.title.slice(0, 40) + "..." : market.title}
      </text>
      <!-- Description -->
      <text x="60" y="250" font-family="Arial, sans-serif" font-size="18" fill="#999">
        ${market.description.length > 80 ? market.description.slice(0, 80) + "..." : market.description}
      </text>
      <!-- Probability bar -->
