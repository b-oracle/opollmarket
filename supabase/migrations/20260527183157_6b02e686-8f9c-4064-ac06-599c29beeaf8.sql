...
BEGIN
  ...
  -- Bonus may cover: market creation fee, boost fee, broadcast fee.
  -- Bonus may NOT cover: liquidity, auto-resolve fee.
  _bonus_eligible := _market_creation_fee_amount + _boost_amount + _broadcast_amount;
  _bonus_for_fee := LEAST(COALESCE(_balance.bonus_balance, 0), _bonus_eligible);
  _main_deduction := _liquidity_amount + _auto_resolve_fee_amount + (_bonus_eligible - _bonus_for_fee);
