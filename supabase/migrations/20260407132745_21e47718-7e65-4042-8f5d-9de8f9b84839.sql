...
-- Drop the old trigger that waited for first prediction
DROP TRIGGER IF EXISTS on_new_bet_referral_reward ON public.transactions;

-- Update handle_new_user to credit referral reward instantly
  ...
  IF v_referred_by IS NOT NULL THEN
    BEGIN
      ...
      IF v_sybil_count = 0 THEN
        ...
        v_reward_amount := COALESCE(v_reward_amount, 5);
