...
DECLARE
  _market RECORD;
  _new_twitter_resource text;
  _new_end_date date;
  _raw_end_date text;
  _sport_match_id text;
  _auto_resolve_deadline timestamptz;
BEGIN
  SELECT * INTO _market FROM markets WHERE id = _market_id;

  IF _market IS NULL THEN
  ...
  -- Twitter/X 5-day limit enforcement
  _new_twitter_resource := NULLIF(_market_data->>'twitter_resource_id', '');
  _raw_end_date := NULLIF(_market_data->>'end_date', '');
  IF _raw_end_date IS NOT NULL THEN
    _new_end_date := _raw_end_date::date;
  ELSE
    _new_end_date := _market.end_date::date;
  END IF;
  
  IF _new_twitter_resource IS NOT NULL AND _new_end_date > (CURRENT_DATE + interval '5 days')::date THEN
    RETURN jsonb_build_object('success', false, 'error', 'Twitter/X markets must resolve within 5 days');
  END IF;

  -- Sports auto-resolve: ensure end_date is at least day after auto_resolve_deadline
  _sport_match_id := COALESCE(NULLIF(_market_data->>'sport_match_id', ''), _market.sport_match_id);
  IF _sport_match_id IS NOT NULL THEN
    _auto_resolve_deadline := COALESCE(
      NULLIF(_market_data->>'auto_resolve_deadline', '')::timestamptz,
      _market.auto_resolve_deadline
    );
    IF _auto_resolve_deadline IS NOT NULL AND _new_end_date <= _auto_resolve_deadline::date THEN
      _new_end_date := (_auto_resolve_deadline::date + 1);
    END IF;
  END IF;
