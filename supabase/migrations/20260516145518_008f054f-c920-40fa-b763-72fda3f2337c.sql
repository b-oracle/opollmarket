...
BEGIN
  ...
  -- Credit registration bonus to EVERY new user, regardless of referral.
  IF v_signup_bonus > 0 THEN
    ...
    -- Audit trail: write a confirmed registration_bonus transaction row
    -- (balance_ledger is populated automatically by trg_log_balance_change).
    IF v_bonus_credited THEN
      ...
      -- In-app notification with deep-link to balance breakdown (handled client-side).
      BEGIN
