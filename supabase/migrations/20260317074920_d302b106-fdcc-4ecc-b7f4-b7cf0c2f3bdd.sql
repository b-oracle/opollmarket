...
BEGIN
  ...
  -- Send Aimtell push notification (broadcast to all Aimtell subscribers)
  BEGIN
    PERFORM extensions.http_post(
      ...
      body := jsonb_build_object(
        'title', NEW.title,
        'body', NEW.message,
        'url', 'https://opollmarket.lovable.app' || _market_url
      )::text,
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || _service_key
  ...
  -- Send WhatsApp notification (includes type for preference filtering)
  BEGIN
    PERFORM extensions.http_post(
