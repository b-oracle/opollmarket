...
/** Convert raw price history into synthetic OHLC candles */
function synthesizeCandles(
  ...
  // Bucket size: divide timeframe into maxCandles buckets
  ...
  for (const pt of sorted) {
    ...
    if (existing) {
      ...
      buckets.set(bucketStart, { o: pt.price, h: pt.price, l: pt.price, c: pt.price, t: bucketStart });
    }
  }

  const candles: OHLCCandle[] = [];
  const sortedBuckets = [...buckets.values()].sort((a, b) => a.t - b.t);
  for (const b of sortedBuckets.slice(-maxCandles)) {
...
function SimpleCandleChart({ ohlcData, priceHistory, entryPrice, assetClass, streamingPrice, chartMs }: Props) {
  const candles = useMemo(() => {
    if (ohlcData && ohlcData.length >= 2) {
      // Take last 60 candles, update the last one with streaming price
  ...
  const { domainMin, domainMax, domainRange, maxVol, gridLevels, ma7Points, ma14Points } = useMemo(() => {
    if (n < 2) return { domainMin: 0, domainMax: 1, domainRange: 1, maxVol: 1, gridLevels: [], ma7Points: null, ma14Points: null };
    ...
    const ma = (period: number) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const y = PRICE_H_PCT * (1 - (avg - dMin) / dRange);
        pts.push(`${x},${y}`);
      }
      return pts.length >= 2 ? pts.join(" ") : null;
    };

    return { domainMin: dMin, domainMax: dMax, domainRange: dRange, maxVol: 1, gridLevels: gl, ma7Points: ma(7), ma14Points: ma(14) };
  }, [candles, n]);
  ...
  const priceY = (p: number) => PRICE_H_PCT * (1 - (p - domainMin) / domainRange);
  const lastCandle = candles[n - 1];
  const lastY = priceY(lastCandle.close);
  const lastPct = (lastY / CHART_H_COMPAT) * 100;
    ...
    <div className="w-full select-none relative" style={{ height: CHART_H }}>
      <svg viewBox={`0 0 100 ${CHART_H}`} preserveAspectRatio="none" className="w-full h-full" style={{ overflow: "visible" }}>
        ...
        {gridLevels.map((level, i) => {
          const y = 6 + (PRICE_H - 8) * (1 - (level - domainMin) / domainRange);
          return <line key={i} x1={0} y1={y} x2={87} y2={y} stroke={GRID} strokeWidth={0.15} strokeDasharray="0.5 0.5" />;
        })}
        ...
        {candles.map((c, i) => {
          ...
          const yFn = (p: number) => 6 + (PRICE_H - 8) * (1 - (p - domainMin) / domainRange);
          const wickTop = yFn(c.high);
          const wickBot = yFn(c.low);
          const bodyTop = yFn(Math.max(c.open, c.close));
          const bodyBot = yFn(Math.min(c.open, c.close));
        ...
        {/* MA7 */}
        {ma7Points && <polyline points={ma7Points} fill="none" stroke={MA7_COLOR} strokeWidth={0.4} strokeLinejoin="round" />}

        {/* MA14 */}
        {ma14Points && <polyline points={ma14Points} fill="none" stroke={MA14_COLOR} strokeWidth={0.4} strokeLinejoin="round" />}

        {/* Entry price */}
        {entryPrice != null && entryPrice >= domainMin && entryPrice <= domainMax && (() => {
          const y = 6 + (PRICE_H - 8) * (1 - (entryPrice - domainMin) / domainRange);
          return <line x1={0} y1={y} x2={100} y2={y} stroke={ENTRY_COLOR} strokeWidth={0.25} strokeDasharray="0.8 0.4" strokeOpacity={0.9} />;
        })()}

        {/* Current price dotted line */}
        {(() => {
          const y = 6 + (PRICE_H - 8) * (1 - (lastCandle.close - domainMin) / domainRange);
          return <line x1={((n - 1) / n) * 100 + 100 / n / 2} y1={y} x2={100} y2={y} stroke={isLastBull ? UP : DOWN} strokeWidth={0.2} strokeDasharray="0.4 0.3" strokeOpacity={0.6} />;
        })()}
      </svg>
      ...
      <div className="absolute right-0 top-0 bottom-0 pointer-events-none" style={{ width: 48 }}>
        {gridLevels.map((level, i) => {
          const yPct = ((6 + (PRICE_H - 8) * (1 - (level - domainMin) / domainRange)) / CHART_H) * 100;
          return (
      ...
      {/* Current price badge */}
      {(() => {
        const yPct = ((6 + (PRICE_H - 8) * (1 - (lastCandle.close - domainMin) / domainRange)) / CHART_H) * 100;
        return (
          <div
            className="absolute right-0 px-1.5 py-0.5 rounded-sm text-[8px] font-bold tabular-nums transition-all duration-300 ease-out"
            style={{ top: `${yPct}%`, transform: "translateY(-50%)", backgroundColor: isLastBull ? UP : DOWN, color: "white" }}
          >
      ...
      {/* Entry price badge */}
      {entryPrice != null && entryPrice >= domainMin && entryPrice <= domainMax && (() => {
        const yPct = ((6 + (PRICE_H - 8) * (1 - (entryPrice - domainMin) / domainRange)) / CHART_H) * 100;
        return (
          <div
            className="absolute right-0 px-1.5 py-0.5 rounded-sm text-[8px] font-bold tabular-nums"
            style={{ top: `${yPct}%`, transform: "translateY(-50%)", backgroundColor: ENTRY_COLOR, color: "white" }}
          >
...
const PRICE_H_PCT = 220 * 0.78; // used in MA calculation
const CHART_H_COMPAT = 220; // for percentage calculations
