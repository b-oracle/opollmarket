                ...
                {step === "confirm" && (
                  <motion.div key="confirm" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    ...
                    <div className="space-y-3">
                      ...
                      <button onClick={handleConfirm} disabled={step === "executing"} className={`w-full ${sideBtnClass} py-3 rounded-xl font-bold text-sm transition-all active:scale-95 disabled:opacity-60 flex items-center justify-center gap-2`}>
                        {step === "executing" ? <><Loader2 className="w-4 h-4 animate-spin" /> Placing...</> : "Confirm Prediction"}
                      </button>
                    </div>
                  </motion.div>
                )}
