...
const SpaceShareSheet = ({ open, onClose, spaceId, spaceTitle, hostName, isLive }: SpaceShareSheetProps) => {
  ...
  const shareText = isLive
    ? `🎙️ Join me LIVE on "${spaceTitle}" — Make your OPinion count, predict now 👇🏽`
    : `🗓️ "${spaceTitle}" is coming soon on OPoll Spaces — Make your OPinion count, predict now 👇🏽`;
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 inset-x-0 z-[101] bg-card rounded-t-2xl border-t border-border p-5 space-y-4 max-h-[70vh] overflow-y-auto"
            >
