...
const Portfolio = () => {
  ...
  const executeSell = useCallback(async () => {
    ...
    try {
      ...
      // 2b. Update market: volume includes sell, liquidity decreases by amount leaving pool
      {
        ...
        if (mkt) {
          // Volume = total buys + total sells
          const newVolume = Number(mkt.volume) + grossProceeds;
          // Liquidity decreases: the net proceeds leave the pool, exit fee stays
          const newLiquidity = Number(mkt.initial_liquidity) + newVolume - grossProceeds + exitFee;
  ...
  if (authLoading) {
    return (
      <div className="min-h-dvh bg-background" style={{ paddingBottom: 'calc(5rem + env(safe-area-inset-bottom))' }}>
        <TopBar />
        <div className="max-w-lg md:max-w-4xl mx-auto px-4 flex items-center justify-center min-h-[60dvh]" style={{ paddingTop: 'calc(5rem + env(safe-area-inset-top, 0px))' }}>
