...
describe("stripCallDeepLinkParams (deep-link URL preservation)", () => {
  const buildIncoming = (extra: string) =>
    `auto_accept=1&call_id=call-abc-123&${extra}`;
  ...
  it("preserves params regardless of position relative to call params", () => {
    // call params are sandwiched between attribution params
    const incoming =
      "utm_source=push&auto_accept=1&utm_medium=notification&call_id=xyz&ref=u1";
  ...
  it("preserves third-party tracking params (fbclid, gclid)", () => {
    ...
    expect(Array.from(result.keys())).not.toContain("auto_accept");
    expect(Array.from(result.keys())).not.toContain("call_id");
  });

  it("preserves duplicate values for the same non-call param key", () => {
    const incoming = `auto_accept=1&call_id=c1&tag=a&tag=b&tag=c`;
  ...
  it("preserves URL-encoded values exactly", () => {
    // ref contains an encoded slash + equals sign
    const incoming = `auto_accept=1&call_id=c1&ref=${encodeURIComponent(
      "campaign/q4=launch",
  ...
  it("returns an empty params when only call params were present", () => {
    const result = stripCallDeepLinkParams("auto_accept=1&call_id=c1");
    expect(result.toString()).toBe("");
  });

  it("accepts a URLSearchParams instance and returns a new instance", () => {
    ...
    expect(input.get("call_id")).toBe("call-abc-123");
    // Result has stripped call params and kept attribution
    expect(result.has("auto_accept")).toBe(false);
    expect(result.has("call_id")).toBe(false);
    expect(result.get("utm_source")).toBe("push");
  });
  ...
  it("matches the round-trip a real navigation would produce", () => {
    // Build a full URL the way the deep link arrives, strip it, and verify
    // the resulting URL matches what react-router's setSearchParams would
    // render in the address bar.
    const url = new URL(
      "https://app.example.com/messages/conv-1?auto_accept=1&call_id=call-9&utm_source=push&utm_campaign=rejoin&ref=u42",
    );
